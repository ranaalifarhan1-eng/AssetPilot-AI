...
class AccountSourceResult(BaseModel):
    balances: List[RawAccountBalance] = Field(default_factory=list)
    status: str = Field(..., description="Source state: available, empty, unavailable, error, not_configured")
...
class PortfolioSummary(BaseModel):
    ...
    valuation_status: str = Field("unconfigured", description="Status: 'complete', 'cached_complete', 'stale_complete', 'partial', 'unavailable', 'unconfigured', 'error'")
    ...
    cached_assets: List[str] = Field(default_factory=list, description="Symbols valued from an unexpired market response cache")
    ...
    source_statuses: Dict[str, str] = Field(default_factory=dict, description="Trading/Funding/Earn source states")
...
class PortfolioStatusResponse(BaseModel):
    ...
    connection_status: str = Field(..., description="Status: 'connected', 'configured_unverified', 'unconfigured', 'error'")
