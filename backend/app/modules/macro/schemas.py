...
class EconomicEvent(BaseModel):
    ...
    source: str = Field(..., description="Authoritative primary source (e.g. Federal Reserve, BLS, BEA, Treasury)")
    source_url: Optional[str] = Field(None, description="Official URL for release or press statement")
    event_name: str = Field(..., description="Human-readable event title (e.g. FOMC Rate Decision, CPI YoY)")
    event_code: str = Field(..., description="Standardized event code (e.g. FED_RATE, CPI_YOY, NFP, GDP_QOQ)")
    ...
    forecast: Optional[float] = Field(None, description="Consensus estimate / forecast, or null if unavailable")
    ...
    surprise_absolute: Optional[float] = Field(None, description="actual - forecast (if both available)")
    surprise_percentage: Optional[float] = Field(None, description="(actual - forecast) / abs(forecast) * 100 (if forecast != 0)")
